...
export default function MarketStatusBar() {
  ...
  return (
    <div className={`terminal-card px-3 sm:px-5 py-3 sm:py-3.5 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3 ${connColor}`}>
      <span className={`h-2 w-2 rounded-full shrink-0 ${connected ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
      <div className="flex-1 min-w-0">
        <div className="text-[9px] sm:text-[10px] font-mono font-bold flex flex-wrap items-center gap-2">
          <span className={`uppercase tracking-wider ${pricesStreaming ? 'text-emerald-300' : 'text-amber-300'}`}>
            {pricesStreaming ? 'Live prices' : 'Last close (market closed)'}
          </span>
          {mounted ? <span className="text-slate-400 font-normal">#{feedPulse}</span> : null}
        </div>
